...
### The bootstrap flag, and why it burns down

The vacuous-pass guard is structural: `emission-1.0.schema.json` sets
`constants: minItems 1` and `counts.total/in_scope: minimum 1`, so an empty
emission is not a representable artifact. That also made an adopter's **first**
build unreachable — a repository generated by `mfc init` has no declarations,
and the generated workflow emits before anything has been written.

`formalization.yaml` carries `bootstrap: true`. While it is set, `validate`,
`bundle` and `lint` accept an empty emission against a copy of the schema with
exactly three constraints dropped (`bootstrap.RELAXED`, asserted by test to be
the only difference), and every E-rule reports **`not_run`** — a green table
over an empty emission would otherwise say "no declaration depends on
`sorryAx`" about a repository with no declarations.

Three properties keep it from being an `--allow-empty` flag with a longer name:

- **It burns down.** `mfc lint` clears it the first time it sees in-scope
  declarations and stamps `bootstrap_cleared_at`. A record carrying that date
  may never set `bootstrap: true` again — `bootstrap.read` raises on the
  combination rather than honouring it.
- **It excuses one situation, not two.** An emission with constants and
  `counts.in_scope: 0` is a *mis-scoped emitter*, and that is a defect on day
  one exactly as it is on day one thousand. Bootstrap does not touch it.
- **A broken emission cannot burn it.** Clearing needs in-scope declarations,
  not merely "not empty", so a bad emitter run cannot cost a repository its
  bootstrap state permanently.

The cleared record is left in the working tree for the operator to commit,
which is what `git diff --exit-code attest/`-style gates are for: clearing the
flag is a commit someone makes, not a fact CI invents.
